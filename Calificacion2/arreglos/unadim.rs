// intercambio
fn intercambiar1(a: &mut [i64; 5], i: usize, j: usize) {
    let aux: i64 = a[i];
    a[i] = a[j];
    a[j] = aux;
}

fn intercambiar2(a: &mut [i64; 10], i: usize, j: usize) {
    let aux: i64 = a[i];
    a[i] = a[j];
    a[j] = aux;
}
...
fn ordIntercambio(arr: &mut [i64; 5]) {
    let mut i: usize = 0;
    let mut j: usize = 0;
...
fn ordSeleccion(arr: &mut [i64; 10]) {
    ...
    while i < (n - 1) {
        ...
        if i != indiceMenor {
            intercambiar2(&mut arr, i, indiceMenor);
        }
